...
export default function Login() {
  const { login,resendEmailVerification,authState} = useAuth();
  ...
  // ---------------------------
   ...
   if (authState.isAuthenticated && authState.user) {
    // console.log("callpost login",authState.user)
  ...
  const handlePostLogin = (user) => {
    if (user.role === "Posp") {
      if (user.email_verification === 0) {
        ...
        if (user.documents_verification === 0) {
          navigate("/posp-document");
        } else if (
          user.documents_verification === 1 &&
          user.training_seconds < 86400
        ) {
        ...
        } else if (user.exam === "fail" && user.can_exam === 0) {
          navigate("/posp-reattampt-mail");
        } else if (user.exam === "pass" && user.can_exam === 0) {
          navigate("/");
        }
      }
    } else if (user.role === "SuperAdmin") {
      navigate("/");
    } else if (user.role === "HR-Head") {
      navigate("/");
  ...
  const handleForgotPassword = async (e) => {
    ...
    try {
      const res = resendEmailVerification(authState.authToken);
         ...
         <div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="space-y-6">
              ...
              {/* SUBMIT */}
              <div>
                <Button className="w-full" size="sm" disabled={authState.loading}>
