...
export function WizardSecond({
  ...
  useEffect(() => {
    if (location.pathname === "/master/hr/create-employee") {
      setFormData({}); // Reset form data for create route
    } else if (
      !isInitialized &&
      initialValues &&
      Object.keys(initialValues).length > 0
    ) {
      setFormData(initialValues); // Use initial values for edit route
      setIsInitialized(true);
    }
  ...
  const handleNext = () => {
    ...
    if (isLastStep) {
      handleSubmit(); // Ensure submission happens immediately
          ...
          <Step
            className={`cursor-pointer ${hasError ? "!bg-red-500 text-white" : ""} ${
              index === correctedActiveStep
                ...
                ? "!bg-blue-400 text-white" // Active and completed steps
                : "!bg-blue-100" // Default steps
            }`}
            key={index}
