...
export function WizardSecond1({
  ...
  const handleSubmit = async () => {
    try {
      onSubmit(formData);
      console.log("Validation amarth :",formData);
          ...
          {allFields.map((field, index) => {
            ...
            return (
              <div
                ...
                style={{ minHeight: field.type === "file" ? '180px' : 'auto' }}
              >
                {field.type !== "checkbox" && (
                  <Typography
                    variant="h6"
                    color="blue-gray"
                    className={`mb-0 font-roboto text-base font-normal ${
                      currentStep?.title === "Process to Upload"
